...
Cross-platform Handshake-first browser core with local HNS proofs, authoritative DNS, an experimental HNS P2P DNS relay and two-hop ODoH requester, RFC 8484 DoH transport, DNSSEC, and DANE diagnostics. Android is the validated shipping baseline; the repository also contains the native iOS 17.0-or-later shell and Apple ABI/build integration. The Apple build and simulator gate uses the stable iOS 26.5 SDK with Xcode 26.5 or 26.6; a signed external-TestFlight device pass is the recommended final iOS release gate and has not been completed.
...
- Implements the direct-locator Handshake P2P ODoH requester as an explicit native runtime policy: a distinct proxy forwards opaque RFC 9230 messages to a Brontide-authenticated target, while DNSSEC and DANE remain local. Shared JavaScript/Rust vectors cover the signed target record and cryptographic encodings, and the real four-node regtest tier exercises the composed browser path with no legacy DoH contact.
