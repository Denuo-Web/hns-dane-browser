...
- Provides a dedicated regtest-only `hnsrTest` Android build whose Rust/JNI client performs native HNSR rendezvous discovery, route sampling and exact lookup, then verifies the complete unnamed route authorization chain. Its Android default-network callback invalidates stale probe generations and reconnects after network restoration; see `docs/experimental-hnsr-native-runbook.md`.
