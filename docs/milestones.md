...
- Activity-scoped native sync scheduler with in-process status snapshots, live-polled first-page sync progress bar with target fallback, separate WebView loading bar, hamburger back/forward/refresh/settings menu, Settings dashboard for diagnostics, cookie options, resolver cache, legal/user agreement, and donations, automatic active first-run catch-up while the browser is visible and peer height or estimated tip is ahead, sync/cache diagnostics status with manual sync triggering, explicit syncing/up-to-date vs peer-failed outcomes, and per-peer sync failure stages.
- HNS omnibox rules.
