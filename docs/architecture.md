...
- `hns-dane`: TLSA record parsing, bounded X.509 SPKI extraction, DANE certificate/SPKI matching, and HNS/WebPKI TLS policy decisions.
- `hns-transport`: bounded HTTP/1.1 origin transport over TCP or rustls TLS, WebPKI fallback, DNSSEC-gated TLSA/DANE certificate policy, fail-closed response framing for unsupported transfer codings or ambiguous lengths, decoded response body streaming to caller-provided writers, and explicit fail-closed rejection for HTTP Upgrade requests before hop-by-hop headers can be stripped. HTTP/2, QUIC, HTTP/3, and HNS WebSocket stream tunneling remain fail-closed.
